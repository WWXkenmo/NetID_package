...
`fate_prob` is a cell fate probability matrix with rows are cells, columns are cell fates, and values are the probabilities of each cell being assigned to each fate. This cell fate probability matrix can be inferred via `cellrank` or `palantir`.

```{r show cell fate, eval=TRUE, echo=TRUE, warning=FALSE,message=FALSE}
head(fate_prob)
```

`pseudotime` is an inferred measure of progression through a biological process, such as differentiation or cell cycle.
```{r show pseudotime, eval=TRUE, echo=TRUE, warning=FALSE,message=FALSE}
head(pseudotime)
```

Then, we assigned cells to specific cell fates based on `fate_prob` using the `LineageClassifier` function, and plugged it into the object `dyn.out`.
