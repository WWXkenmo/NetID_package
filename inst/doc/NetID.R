...
## ----Validate cell type label, eval=TRUE, echo=TRUE, warning=FALSE------------
...
## ----Run NetID (set path), eval=FALSE, echo=TRUE, warning=FALSE---------------
#  dyn.out <- RunNetID(sce, regulators = TF[,1], targets = TF[,1],
#                      netID_params = list(normalize=FALSE),
...
## ----Run NetID with SNN, eval=FALSE, echo=TRUE, warning=FALSE-----------------
...
## ----check FateDynamic parameters, eval=FALSE, echo=TRUE, warning=FALSE-------
#  ?check_global_params
#  ?check_palantir_params
#  ?check_cellrank_params

## ----Run FateDynamic (SCENT), eval=FALSE, echo=TRUE, warning=FALSE------------
#  FateDynamic(sce,
#              global_params = list(cluster_label = "celltype",
#                                   min_counts = 10 ,
#                                   nhvgs = 3000,
...
## ----plot cell fate probability, eval=TRUE, echo=TRUE, warning=FALSE----------
...
## ----check network, eval=TRUE, echo=TRUE, warning=FALSE-----------------------
GRN$grn[[1]][1:10,1:10]

## ----statistical test of the module, eval=TRUE, echo=TRUE, warning=FALSE------
result = ModuleFinder(GRN,"erythroblasts_stage2",c("Gata1"),
                      method = "spinglass")
result

## ----statistical test of the module2, eval=TRUE, echo=TRUE, warning=FALSE-----
result2 = ModuleFinder(GRN,"neutrophils",c("Gata1"),
                       method = "spinglass")
result2

## ----identify smallar module, eval=TRUE, echo=TRUE, warning=FALSE-------------
result = ModuleFinder(GRN,"erythroblasts_stage2",c("Gata1"),
                      method = "spinglass",gamma = 0.4)
result

## ----visualize the module, eval=TRUE, echo=TRUE, warning=FALSE----------------
