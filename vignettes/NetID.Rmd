...
Before using NetID package, some python modules is required, please install following modules before running following tutorial

```{r install require module, eval=FALSE, echo=TRUE, message=FALSE, warning=FALSE}
...
```{r Validate cell type label, eval=TRUE, echo=TRUE, warning=FALSE}
...
```{r Run NetID (set path), eval=FALSE, echo=TRUE, warning=FALSE}
dyn.out <- RunNetID(sce, regulators = TF[,1], targets = TF[,1],
                    netID_params = list(normalize=FALSE), 
...
```{r Run NetID (geosketch results), eval=FALSE, echo=TRUE, warning=FALSE}
...
```{r check FateDynamic parameters, eval=FALSE, echo=TRUE, warning=FALSE}
?check_global_params
...
```{r Run FateDynamic (set terminal state), eval=FALSE, echo=TRUE, warning=FALSE}
FateDynamic(sce,
            global_params = list(cluster_label = "celltype",
                                 min_counts = 10 ,
                                 nhvgs = 3000,
...
In this case, proceed to run NetID once more, considering the availability of dynamic information with dynamicInfer = TRUE.
...
```{r plot cell fate probability, eval=TRUE, echo=TRUE, warning=FALSE}
